...
class ProfileWithRepliesQuery
{
    ...
    public function baseQuery(): Builder
    {
        ...
        $posts = Post::query()
            ->where(fn($q) => $q->whereBelongsTo($this->subject, 'profile')->whereNull('parent_id'))
            ->orWhereHas('replies', fn($q) => $q->whereBelongsTo($this->subject, 'profile'))
            ->with([
                'profile',
                'repostOf' => fn($q) => $q->withCount(['likes', 'reposts', 'replies'])->with('profile'),
                'repostOf.profile',
                'parent.profile',
                'replies' => fn($q) => $q->whereBelongsTo($this->subject, 'profile')->with('profile')->oldest()
            ])
